...
def main():
    ...
    ms_side = np.sqrt(5e-4)             # q0 = 5 cm^2
    mx0 = 0.5 - ms_side / 2; my0 = H - ms_side

    mpm.set_configuration(domain=[H, H], background_damping=0.0, alphaPIC=0.85,
        mapping='USL', shape_function='QuadBSpline', gravity=[0., -G],
        material_type='TwoFluidSediment', velocity_projection='PIC')
    mpm.set_solver({'Timestep': 4e-5, 'SimulationTime': 1.0, 'SaveInterval': 0.05,
                    'SavePath': 'mud_twofluid_08C1'})
    mpm.memory_allocate(memory={'max_material_number': 2, 'max_particle_number': 200000,
        'verlet_distance_multiplier': 1., 'max_constraint_number': {'max_reflection_constraint': 50000}})
    ...
    base = {'Density': rho_f0, 'SolidDensity': rho_s, 'ParticleDiameter': dp,
            ...
            'FilterScale': 0.0025, 'cL': 0.5,
            'SedimentDiffusivity': 1e-3}
    water = dict(base); water.update({'MaterialID': 1, 'Concentration': 0.0})
    mud = dict(base); mud.update({'MaterialID': 2, 'Concentration': alpha_s0})
    mpm.add_material(model='TwoFluidSediment', material=water)
    mpm.add_material(model='TwoFluidSediment', material=mud)
    ...
    mpm.add_region([
        ...
        {'Name': 'mud', 'Type': 'Rectangle2D', 'BoundingBoxPoint': [mx0, my0], 'BoundingBoxSize': [ms_side, ms_side], 'ydirection': [0., 1.]}
    ])
    mpm.add_body({'Template': [
        {'RegionName': 'full_water', 'nParticlesPerCell': 1, 'BodyID': 0, 'MaterialID': 1, 'ParticleStress': cs, 'InitialVelocity': [0., 0.], 'FixVelocity': ['Free', 'Free']},
        {'RegionName': 'mud', 'nParticlesPerCell': 1, 'BodyID': 0, 'MaterialID': 2, 'ParticleStress': cs, 'InitialVelocity': [0., 0.], 'FixVelocity': ['Free', 'Free']}
    ]})
